            ...
            <div className="max-w-7xl mx-auto px-6">
                <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
                    <div className="max-w-2xl">
                        ...
                        <h2 className="text-4xl font-bold text-primary dark:text-white mb-6">Expert <span className="text-accent">Knowledge</span> for Better Decisions</h2>
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {topics.map((topic, idx) => (
                        <div key={idx} className="group p-8 rounded-[2.5rem] bg-cream dark:bg-white/5 border border-primary/5 hover:border-accent/20 transition-all duration-500">
                            <div className={`w-14 h-14 ${topic.bg} ${topic.color} rounded-2xl flex items-center justify-center mb-8`}>
                                <topic.icon size={28} />
                            </div>
                            <div className="inline-block px-3 py-1 rounded-full bg-white dark:bg-white/10 text-[10px] font-bold uppercase tracking-widest text-accent mb-4 border border-primary/5 dark:border-white/5">
                                {topic.tip}
                            </div>
                            <h3 className="text-2xl font-bold text-primary dark:text-white mb-4">{topic.title}</h3>
                            <p className="text-primary/60 dark:text-white/60 mb-8 leading-relaxed text-sm">
                                {topic.desc}
                            </p>
                            <button className="flex items-center gap-2 text-sm font-bold text-primary dark:text-white group-hover:text-accent transition-colors">
                ...
                <div className="mt-16 p-8 rounded-3xl bg-primary text-white flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
                    ...
                    <button className="relative z-10 bg-accent text-primary px-8 py-4 rounded-xl font-bold hover:scale-105 transition-transform flex items-center gap-2">
                        Get Knowledge Pack
                        <Lightbulb size={18} />
                    </button>
                    {/* Decorative glow */}
